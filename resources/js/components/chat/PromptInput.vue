<script setup lang="ts">
    ...
    const actionLabel = computed(() =>
        props.cancelable ? 'Cancel message' : 'Send message'
    );

    const canSubmit = computed(() => {
        return !props.disabled
            && !props.cancelable
            && messageValue.value.trim().length > 0;
    })
...
<template>
    <div class="">
        ...
        <div
            class="relative rounded-3xl border bg-card shadow-sm transition-shadow hover:shadow-md focus-within:ring focus-within:ring-ring">
            <!-- Textarea -->
            <Textarea id="message-input" name="message" v-model="messageValue" :placeholder="props.placeholder"
                :max-lines="props.maxLines" :aria-label="props.label" @keydown="handleKeyDown"
            ...
            <div class="flex items-center gap-3 px-4 pb-4">
                ...
                <button type="button" @click="onAction" :disabled="props.disabled && !props.cancelable"
